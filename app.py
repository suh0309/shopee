...
# 4. CLV Prediction (fixed)
    ...
    bgf.fit(df['frequency'], df['recency'], T_series)

    ggf = GammaGammaFitter(penalizer_coef=0.0)
    ggf.fit(df['frequency'], df['monetary'])

    horizon = st.slider("Forecast horizon (days):", 30, 180, 90, step=30)
    T_series_h = pd.Series(horizon, index=df.index)
    clv = bgf.customer_lifetime_value(
        ggf,
        df['frequency'], df['recency'], df['monetary'], time=horizon
    )
...
# 6. Next-Purchase Regression
    ...
    model = RandomForestRegressor(50, random_state=42).fit(customers[[x]], y)
...
# 8. Market Response
    ...
    fig = px.scatter(df_mr, x='Ad Spend', y='Response', title="Market Response Model")
    st.plotly_chart(fig, use_container_width=True)
...
# 10. Association Rule Mining + Clustering dropdown
elif section == "Association Rules":
    st.header("Association Rule Mining")

    # Clustering dropdown
    st.subheader("Optional RFM Clustering")
    k = st.selectbox("Number of clusters:", 2, 6, 3)
    rfm = customers[['total_orders','avg_order_value','days_since_last_order']].copy()
    ...
    elbow_df = pd.DataFrame({'min_support':supports,'itemsets':counts})
    fig_elbow = px.line(elbow_df, x='min_support', y='itemsets',
                        labels={'min_support':'Support','itemsets':'# Itemsets'},
                        title="Elbow Curve: Support vs Itemsets")
