...
# 4. CLV Prediction (Fixed)
    ...
    df = customers[customers['total_orders'] > 1].copy()  # only >1 order
    ...
    # Pass T as a Series, not scalar
    T_initial = pd.Series(60, index=df.index)
    bgf = BetaGeoFitter(penalizer_coef=0.0)
    bgf.fit(df['frequency'], df['recency'], T=T_initial)
...
# 7. Sales Forecast
    ...
    fig = px.line(df_sales, x='Day', y='Sales', title="Simulated Sales Trend")
    st.plotly_chart(fig, use_container_width=True)
...
# 9. Uplift Modeling
    ...
    fig = px.histogram(df_up, x='outcome', color='treatment',
                       ...
                       title="Uplift: Purchase by Group")
    st.plotly_chart(fig, use_container_width=True)

# 10. Association Rule Mining + Clustering
elif section == "Association Rules":
    st.header("Association Rule Mining")

    # Clustering dropdown
    st.subheader("RFM Clustering Overview")
